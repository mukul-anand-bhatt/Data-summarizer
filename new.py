...
if manipulation_option == 'Insert a Row':
    row_index = st.number_input('Enter the row index to insert at', min_value=0, max_value=len(df), value=len(df))
    val = st.text_input('Enter the list of values in a sequence, separated by commas')
    if st.button('Insert Row'):
        df.loc[row_index] = val.split(',')
        st.write('New row inserted')
        st.write(df)

elif manipulation_option == 'Delete a Row(s)':
    row_index = st.number_input('Enter the row index to delete', min_value=0, max_value=len(df)-1, value=0)
    ...
    if st.button('Insert Column'):
        df[column_name] = val.split(',')
        st.write('New column inserted')
        st.write(df)

elif manipulation_option == 'Delete a column(s)':
    column_name = st.selectbox('Select a column to delete', df.columns)
    ...
    row_index = st.number_input('Enter the row index to modify', min_value=0, max_value=len(df)-1, value=0)
    column_name = st.selectbox('Select a column to modify', df.columns)
